...
% fsname = 'spm';
% dataset = ['h_train_',fsname,'_v200'];
% load(['./data/',dataset,'.mat']);
% dataset2 = ['h_test_',fsname,'_v200'];
...
% fsname = 'gmhi';
% features = 6;
% dataset = 'h_train_gmhi_6';
% load(['./data/',dataset,'.mat']);
% dataset2 = 'h_test_gmhi_6';
% load(['./data/',dataset2,'.mat']);
...
X_train = X;
for i = 1:size(X,1) %1-行数
    for j = 1:size(X,2)
        if X(i,j)<=1
            X_train(i,j) = log2(mtds*(X(i,j))+epss);   %mtds=2         
        else
            X_train(i,j) = sqrt(X(i,j));  %平方根
        end
    end
end
...
% fea = log(fea+eps);
fea2 = X_test;%log(+eps);
[fea_h,fea_n,meanxapp,stdxapp] = normalizemeanstd(fea_h,fea_n);%均值、标准差
fea_h50=fea_h;
fea_n50=fea_n;
[~,fea2,~,~] = normalizemeanstd(fea_h,fea2,meanxapp,stdxapp);
%%
...
if ~iscell(feature_name)        %确定输入是否为元胞数组
    feature_name=cellstr(feature_name); %转换为字符向量元胞数组
    fea_names = feature_name;
end

N = size(fea,2);
%% 
load(['D:\原电脑\研一下\PCA_results\PCA_result_all_',fsname,'_315.mat'])
% load('F:\MATLAB\AEFS-master\PCA_results\PCA_result_all_mic_314.mat')
% features = 10:10:200;
% features = 50;
...
% load('matlab92.mat','meta_iPCA')
load('h_train_all_meta9_4347.mat')
% load('GMHI_output_4347.mat')
% meta_all = [meta_age;meta_bmi;meta_TRIG;meta_CHOL;meta_fbg;meta_HDLC;meta_LDLC;num2cell(meta_iPCA)];
% for i = 1:size(meta_all,1)-1
%     for j = 1:size(meta_all,2)
%         try
%             meta_all{i,j} = str2num(meta_all{i,j});
%         catch
%             meta_all{i,j} = NaN;
%         end
%         if isempty(meta_all{i,j})
%             meta_all{i,j} = NaN;
%         end
%     end
% end
...
% [R,Pval,H] = corrplot(dat_table(:,ind_set),'varNames',sets(ind_set),'type','Spearman','rows','pairwise','testR','on','alpha',0.01)
% cellfun(@(x) x.*38.67,meta_HDLC,'UniformOutput',false)
meta_age=num2cell(meta_age);
meta_bmi=num2cell(meta_bmi);
meta_TRIG=num2cell(meta_TRIG);
meta_CHOL=num2cell(meta_CHOL);
meta_fbg=num2cell(meta_fbg);
meta_HDLC=num2cell(meta_HDLC);
meta_LDLC=num2cell(meta_LDLC);
meta_all = [meta_age;meta_bmi;meta_TRIG;meta_CHOL;meta_fbg;meta_HDLC;meta_LDLC;num2cell(alpha);num2cell(prev);num2cell(richness);num2cell(meta_iPCA)];
% for i = 1:size(meta_all,1)-1
%     for j = 1:size(meta_all,2)
%         try
%             meta_all{i,j} = str2num(meta_all{i,j});
...
dat_table = cell2num(meta_all)';%
ind_set = [1:11];
sets = {'AGE','BMI','TRIG','CHOL','FBG','HDLC','LDLC','Shannon','prev','richness','hiPCA'};%dat_table(:,6) = dat_table(:,6)*38.67;

[R,Pval,H] = corrplot(dat_table(:,ind_set),'varNames',sets(ind_set),'type','Spearman','rows','pairwise','testR','on','alpha',0.001)
% save("GMHI-RPH.mat", "Pval", "R", "H");
% save("hiPCA-RPH.mat", "Pval", "R", "H");
% save("hiPCA-RPH-92.mat", "Pval", "R", "H");
%[R,Pval,H] = corrplot(dat_table(:,ind_set),'varNames',sets(ind_set),'type','Pearson','rows','pairwise','tail','left','testR','on','alpha',0.05)
1;
% for i = 1:length(meta_bmi)
%    try
%        meta_bmi{1,i} = str2num(meta_bmi{1,i}); 
%    catch
%        meta_bmi{1,i} = NaN;
%    end
%    if isempty(meta_bmi{1,i})
%        meta_bmi{1,i} = NaN;
%    end
% end
%% Regional pies
% load('h_train_all_meta9.mat')
...
idx_aa = find(strcmp(meta_train, 'advanced adenoma' )); 

load('GMHI_output_4347.mat')
Phenotype=Phenotype';
idx_train_healthy_GMHI = find(strcmp(Phenotype, 'Healthy' ));
idx_train_unhealthy_GMHI = setdiff(idx_all,idx_train_healthy_GMHI);
idx_ACVD_GMHI = find(strcmp(Phenotype, 'ACVD' ));                                %id_1 = strcmp(meta_train, 'ACVD' );
idx_CRC_GMHI = find(strcmp(Phenotype, 'CRC' ));                                 % id_2 = strcmp(meta_train, 'CRC' );
idx_CD_GMHI = find(strcmp(Phenotype, 'Crohns disease' ));                      % id_3 = strcmp(meta_train, 'Crohns disease');
idx_IGT_GMHI = find(strcmp(Phenotype, 'IGT' ));                                 %id_4 = strcmp(meta_train,'IGT');
idx_Obesity_GMHI = find(strcmp(Phenotype, 'Obesity' ));                        % id_5 = strcmp(meta_train,'Obesity');
idx_Overweight_GMHI = find(strcmp(Phenotype, 'Overweight' ));                  % id_6 = strcmp(meta_train,'Overweight');
idx_RA_GMHI = find(strcmp(Phenotype, 'Rheumatoid Arthritis' ));                % id_7 = strcmp(meta_train,'Rheumatoid Arthritis');
idx_SA_GMHI = find(strcmp(Phenotype, 'Symptomatic atherosclerosis' ));        %  id_8 = strcmp(meta_train,'Symptomatic atherosclerosis');
idx_T2D_GMHI = find(strcmp(Phenotype, 'T2D' ));                                %id_9 = strcmp(meta_train,'T2D');   
idx_UC_GMHI = find(strcmp(Phenotype, 'Ulcerative colitis' ));                  % id_10 = strcmp(meta_train,'Ulcerative colitis');  
idx_Underweight_GMHI = find(strcmp(Phenotype, 'Underweight' ));                % id_11 = strcmp(meta_train,'Underweight');                      
idx_aa_GMHI = find(strcmp(Phenotype, 'advanced adenoma' )); 
F_4=GMHI(mask_h);F_5=GMHI(mask_n);

% id_12 = strcmp(meta_train,'advanced adenoma');  
% jj = 1;
...
%% 
GMHI_Heathy = F_4;
FAR_Healthy_GMHI = length(find(GMHI_Heathy<0))/length(F_4);
FDR_Healthy_GMHI = length(find(GMHI_Heathy>0))/length(F_4);
GMHI_Heathy=GMHI_Heathy';

c_ACVD_GMHI=ismember(idx_train_unhealthy_GMHI,idx_ACVD_GMHI);
GMHI_ACVD = F_5(c_ACVD_GMHI);
FDR_ACVD_GMHI = length(find(GMHI_ACVD<0))/length(F_5(c_ACVD_GMHI));
GMHI_ACVD=GMHI_ACVD';

c_CRC_GMHI=ismember(idx_train_unhealthy_GMHI,idx_CRC_GMHI);
GMHI_CRC = F_5(c_CRC_GMHI);
FDR_CRC_GMHI = length(find(GMHI_CRC<0))/length(F_5(c_CRC_GMHI));
GMHI_CRC=GMHI_CRC';

c_CD_GMHI=ismember(idx_train_unhealthy_GMHI,idx_CD_GMHI);
GMHI_CD = F_5(c_CD_GMHI);
FDR_CD_GMHI = length(find(GMHI_CD<0))/length(F_5(c_CD_GMHI));
GMHI_CD=GMHI_CD';

c_IGT_GMHI=ismember(idx_train_unhealthy_GMHI,idx_IGT_GMHI);
GMHI_IGT = F_5(c_IGT_GMHI);
FDR_IGT_GMHI = length(find(GMHI_IGT<0))/length(F_5(c_IGT_GMHI));
GMHI_IGT=GMHI_IGT';

c_Obesity_GMHI=ismember(idx_train_unhealthy_GMHI,idx_Obesity_GMHI);
GMHI_Obesity = F_5(c_Obesity_GMHI);
FDR_Obesity_GMHI = length(find(GMHI_Obesity<0))/length(F_5(c_Obesity_GMHI));
GMHI_Obesity=GMHI_Obesity';

c_Overweight_GMHI=ismember(idx_train_unhealthy_GMHI,idx_Overweight_GMHI);
GMHI_Overweight = F_5(c_Overweight_GMHI);
FDR_Overweight_GMHI = length(find(GMHI_Overweight<0))/length(F_5(c_Overweight_GMHI));
GMHI_Overweight=GMHI_Overweight';

c_RA_GMHI=ismember(idx_train_unhealthy_GMHI,idx_RA_GMHI);
GMHI_RA = F_5(c_RA_GMHI);
FDR_RA_GMHI = length(find(GMHI_RA<0))/length(F_5(c_RA_GMHI));
GMHI_RA=GMHI_RA';

c_SA_GMHI=ismember(idx_train_unhealthy_GMHI,idx_SA_GMHI);
GMHI_SA = F_5(c_SA_GMHI);
FDR_SA_GMHI = length(find(GMHI_SA<0))/length(F_5(c_SA_GMHI));
GMHI_SA=GMHI_SA';

c_T2D_GMHI=ismember(idx_train_unhealthy_GMHI,idx_T2D_GMHI);
GMHI_T2D = F_5(c_T2D_GMHI);
FDR_T2D_GMHI = length(find(GMHI_T2D<0))/length(F_5(c_T2D_GMHI));
GMHI_T2D =GMHI_T2D';

c_UC_GMHI=ismember(idx_train_unhealthy_GMHI,idx_UC_GMHI);
GMHI_UC = F_5(c_UC_GMHI);
FDR_UC_GMHI = length(find(GMHI_UC<0))/length(F_5(c_UC_GMHI));
GMHI_UC=GMHI_UC';

c_Underweight_GMHI=ismember(idx_train_unhealthy_GMHI,idx_Underweight_GMHI);
GMHI_Underweight = F_5(c_Underweight_GMHI);
FDR_Underweight_GMHI = length(find(GMHI_Underweight<0))/length(F_5(c_Underweight_GMHI));
GMHI_Underweight=GMHI_Underweight';

c_aa_GMHI=ismember(idx_train_unhealthy_GMHI,idx_aa_GMHI);
GMHI_aa = F_5(c_aa_GMHI);
FDR_aa_GMHI = length(find(GMHI_aa<0))/length(F_5(c_aa_GMHI));
GMHI_aa=GMHI_aa';
...
g21 = repmat({'Healthy'},length(idx_train_healthy_GMHI),1);
g22 = repmat({'ACVD'},length(idx_ACVD_GMHI),1);
g23 = repmat({'CRC'},length(idx_CRC_GMHI),1);
g24 = repmat({'CD'},length(idx_CD_GMHI),1);
g25 = repmat({'IGT'},length(idx_IGT_GMHI),1);
g26 = repmat({'Obesity'},length(idx_Obesity_GMHI),1);
g27 = repmat({'Overweight'},length(idx_Overweight_GMHI),1);
g28 = repmat({'RA'},length(idx_RA_GMHI),1);
g29 = repmat({'SA'},length(idx_SA_GMHI),1);
g30 = repmat({'T2D'},length(idx_T2D_GMHI),1);
g31 = repmat({'UC'},length(idx_UC_GMHI),1);
g32 = repmat({'Underweight'},length(idx_Underweight_GMHI),1);
g33 = repmat({'advanced adenoma'},length(idx_aa_GMHI),1);
gg = [g21; g22; g23; g24; g25; g26; g27; g28; g29; g30; g31; g32; g33];
HH = boxplot([GMHI_Heathy;GMHI_ACVD;GMHI_CRC;GMHI_CD;GMHI_IGT;GMHI_Obesity;GMHI_Overweight;GMHI_RA;GMHI_SA;GMHI_T2D;GMHI_UC;GMHI_Underweight;GMHI_aa],gg);
xtickangle(45)
ylabel('GMHI')
title('Compare cohorts')
pvs = zeros(1,12);
[h,pvs(1),ci,stats] = ttest2(GMHI_Heathy,GMHI_ACVD);
[h,pvs(2),ci,stats] = ttest2(GMHI_Heathy,GMHI_CRC);
[h,pvs(3),ci,stats] = ttest2(GMHI_Heathy,GMHI_CD);
[h,pvs(4),ci,stats] = ttest2(GMHI_Heathy,GMHI_IGT);
[h,pvs(5),ci,stats] = ttest2(GMHI_Heathy,GMHI_Obesity);
[h,pvs(6),ci,stats] = ttest2(GMHI_Heathy,GMHI_Overweight);
[h,pvs(7),ci,stats] = ttest2(GMHI_Heathy,GMHI_RA);
[h,pvs(8),ci,stats] = ttest2(GMHI_Heathy,GMHI_SA);
[h,pvs(9),ci,stats] = ttest2(GMHI_Heathy,GMHI_T2D);
[h,pvs(10),ci,stats] = ttest2(GMHI_Heathy,GMHI_UC);
[h,pvs(11),ci,stats] = ttest2(GMHI_Heathy,GMHI_Underweight);
[h,pvs(12),ci,stats] = ttest2(GMHI_Heathy,GMHI_aa);
groups={{'Healthy','ACVD'},...
 		{'Healthy','CRC'},... %note you can mix and match notions
		{'Healthy','CD'},...
        {'Healthy','IGT'},...
...
figure
cc = categorical({'Healthy','ACVD','CRC','CD','IGT','Obesity','Overweight','RA','SA','T2D','UC','Underweight','advanced adenoma'});
FDRss = [FDR_Healthy_GMHI FDR_ACVD_GMHI FDR_CRC_GMHI FDR_CD_GMHI FDR_IGT_GMHI FDR_Obesity_GMHI FDR_Overweight_GMHI FDR_RA_GMHI FDR_SA_GMHI FDR_T2D_GMHI FDR_UC_GMHI FDR_Underweight_GMHI FDR_aa_GMHI];
bb=bar(cc,FDRss);
bb.FaceColor = 'flat';
bb.LineWidth = 1.5
bb.CData(4,:) = [.5 0 .5];
ylabel('GMHI accuracy ratio')
